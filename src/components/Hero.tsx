...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Responsive Layout Overlaid on Full Screen Router Slide (Top & Bottom on Mobile, Side-by-Side on Desktop) */}
      <div className="relative z-20 w-full max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-8 md:gap-0 h-full pt-28 pb-16 md:py-12">
        
        {/* Top Headline on Mobile (Left Side on Desktop) */}
        <div className="w-full md:flex-1 text-left">
          <AnimatePresence mode="wait">
            ...
            >
              <span className="font-mono text-xs md:text-sm tracking-[0.2em] text-gray-800 block mb-2 md:mb-3 uppercase font-extrabold">
        ...
        {/* Bottom Headline on Mobile (Right Side on Desktop) */}
        <div className="w-full md:flex-1 text-left md:text-right flex justify-start md:justify-end mt-auto md:mt-0">
          <AnimatePresence mode="wait">
            <motion.div
              key={currentSlideIndex}
              initial={{ opacity: 0, y: 20 }}
